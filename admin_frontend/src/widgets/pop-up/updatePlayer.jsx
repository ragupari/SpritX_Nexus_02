...
export function UpdatePlayer({ isOpen, onClose, onSave, player }) {
  ...
  const handleUpdate = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:3000/api/players/${Player_ID}`, {
        method: "PUT",
        headers: {
      ...
      onSave(updatedPlayer); // Update player list in the parent component
      onClose();
    } catch (error) {
